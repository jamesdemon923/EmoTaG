...
class GridRenderer(nn.Module):
    ...
    def __init__(self, bound=1.0, coord_center=[0.0, 0.0, 0.0], keep_sigma=False):
        super().__init__()
    ...
    def create_encoder(self):
        self.encoder_x, self.in_dim_x = get_encoder('hashgrid', input_dim=3, num_levels=self.num_levels, level_dim=self.level_dim, base_resolution=self.base_resolution, log2_hashmap_size=self.table_size, desired_resolution=self.desired_resolution * self.bound.cpu())
    ...
    def density(self, x, enc_x=None):
        ...
        geo_feat = h[..., 1:]
        if self.keep_sigma:
            self.sigma_results_static = {'sigma': sigma, 'geo_feat': geo_feat}
        return {'sigma': sigma, 'geo_feat': geo_feat}

    def get_params(self, lr, lr_net, wd=0):
        params = [{'params': self.encoder_x.parameters(), 'name': 'neural_encoder', 'lr': lr}, {'params': self.sigma_net.parameters(), 'name': 'neural_sigma', 'lr': lr_net, 'weight_decay': wd}, {'params': self.color_net.parameters(), 'name': 'neural_color', 'lr': lr_net, 'weight_decay': wd}]
        return params
